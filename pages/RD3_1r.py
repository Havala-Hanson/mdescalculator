...
def render_inputs(design):
    ...
    cutoff = st.number_input(
        ...
        value=0.0,
        step=0.1,
        help = "The cutoff value on the running variable that determines treatment assignment. Observations with running variable values above (or below, depending on treatment side) this cutoff receive treatment. Default: 0.0.",
    )
    ...
    r2_level3 = st.number_input(
        ...
        value=0.0,
        step=0.05,
        help = "Proportion of variance in the outcome explained by level-3 covariates. This can increase power by reducing residual variance at the highest level of clustering. Default: 0.0.",
    )
    ...
    if outcome_type == "binary":
        ...
        outcome_sd = st.number_input(
            ...
            value=1.0,
            step=0.1,
            help = "Standard deviation of the outcome variable in raw units (e.g., dollars, test score points). This is used to convert the standardized MDES into raw units. If you don't have an estimate, you can use 1.0, and the MDES will be interpreted in standard deviation units.",
        )
